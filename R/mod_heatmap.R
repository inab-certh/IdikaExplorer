  ...
  shiny::tagList(
    ...
    shiny::fluidRow(
      shiny::column(12,
        ...
        shiny::helpText(
          "Each cell shows prescriptions of a given ATC code (or group) per patient",
          "diagnosed with a given ICD-10 condition (or group).",
          "The denominator is the total number of unique patients carrying",
          "that ICD-10 code / group in the current filtered dataset.",
          shiny::br(),
          "Define custom groups in the",
          shiny::strong("Overall data"),
          "tab to collapse multiple codes into a single row / column."
        )
      )
    ),
    ...
    shiny::fluidRow(
      ...
      shiny::column(3,
        ...
        shiny::downloadButton(
          ns("download_data"), "Download data (CSV)",
...
#'
#' @param id            Module id.
#' @param data_reactive Reactive returning the (already-grouped) filtered data.
#' @param groups        Reactive list(atc=…, icd10=…) from mod_groups_server().
#'                      Used here only for axis label decoration and click
#'                      drill-down; the actual code→group mapping has already
#'                      been applied upstream in grouped_display_data.
...
mod_heatmap_server <- function(id, data_reactive, groups, con) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    atc_lookup <- shiny::reactive({
      DBI::dbGetQuery(
        con, "SELECT atc_code, atc_name FROM atc_codes ORDER BY atc_code"
      ) |>
    ...
    icd10_lookup <- shiny::reactive({
      DBI::dbGetQuery(
        con, "SELECT code, short_description FROM icd_10_cm ORDER BY code"
      ) |>
        dplyr::distinct(code, .keep_all = TRUE)
    })

    # ── Label helpers ─────────────────────────────────────────────────────────
    # Group-named codes are returned verbatim; raw codes get "CODE – Name".
    ...
    # The input data already has group names in atc_code / icd10 columns
    # (applied upstream). No second round of apply_groups_to_codes needed.
    heatmap_data <- shiny::reactive({
      ...
      # atc_key / icd10_key are now the same as atc_code / icd10
      # (groups already collapsed upstream)
      data_keyed <- data |>
        dplyr::rename(atc_key = atc_code, icd10_key = icd10)

      # Denominator: unique patients per ICD-10 key
      icd10_patients <- data_keyed |>
    ...
    shiny::observeEvent(
      plotly::event_data("plotly_click", source = ns("heatmap_click")),
      {
        ...
        grps <- groups()
        atc_key_val   <- as.character(row$atc_key[1])
        icd10_key_val <- as.character(row$icd10_key[1])

        selected_cell(list(
          atc_key        = atc_key_val,
          icd10_key      = icd10_key_val,
          atc_label      = click$x,
          icd10_label    = click$y,
          is_atc_group   = atc_key_val   %in% names(grps$atc),
          is_icd10_group = icd10_key_val %in% names(grps$icd10)
        ))
      }
    )
    ...
    output$click_detail_table <- DT::renderDataTable({
      ...
      # In pre-grouped data the atc_code / icd10 columns already hold either
      # raw codes or group names.  Just filter by the key value directly.
      # For group keys we additionally expand to underlying raw codes using
      # the original group patterns so the detail table can show granular rows.
      if (cell$is_atc_group) {
        # Find rows whose atc_code equals the group name (already collapsed)
        atc_sel <- data$atc_code == cell$atc_key
      } else {
        atc_sel <- data$atc_code == cell$atc_key
      }

      if (cell$is_icd10_group) {
        icd10_sel <- data$icd10 == cell$icd10_key
      } else {
        icd10_sel <- data$icd10 == cell$icd10_key
      }

      detail <- data[atc_sel & icd10_sel, ] |>
        dplyr::group_by(
          prescription_insert_year, region, age_group, sex
        ) |>
        dplyr::summarise(
    ...
    output$download_data <- shiny::downloadHandler(
      ...
      content  = function(file) {
        ...
        readr::write_csv(
          df |>
            dplyr::select(
              icd10_key, icd10_label, atc_key, atc_label,
