  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$summary_table <- DT::renderDataTable({
      ...
      if (length(grouping_vars) == 0) {
        ...
        summary_data <- summary_data |>
          dplyr::mutate(
            prescriptions_per_100k = (total_prescriptions / population) * 1e5,
            patients_per_100k      = (unique_patients / population) * 1e5,
            intensity              = total_prescriptions / unique_patients
          ) |>
      ...
      column_name_map <- c(
        ...
        "intensity"                = "Prescriptions per Patient"
      )
      ...
      # Add styled bars for prescriptions per 100k
      ...
      # Add styled bars for patients per 100k
      ...
      # Add styled bars for prescriptions per patient
      if ("Prescriptions per Patient" %in% names(summary_data)) {
        dt <- dt |>
          DT::formatStyle(
            "Prescriptions per Patient",
            background = DT::styleColorBar(
              c(0, max(summary_data$`Prescriptions per Patient`, na.rm = TRUE)),
          ...
          DT::formatRound(columns = "Prescriptions per Patient", digits = 2, mark = ",")
      }
